...
method="yandex/apt"
if [ -f "$input" ]; then
    cp "$input" yandex/local/yandex-browser-stable.deb
...
test_image(){
    ...
    if [ -d "$tests_dir" ]; then
        pushd "$tests_dir"
        mvn clean test -Dgrid.connection.url="http://localhost:4445/" -Dgrid.browser.name=yandex -Dgrid.browser.version=$2
        popd
    else
